...
path = './FrcSub/'
ratio1, ratio2, ratio3 = 0.6, 0.6, 0.25
...
for i in range(TRAIN_NUM):
    ...
    loss_func = nn.CrossEntropyLoss()
    meta_learner = MetaLearner('ACDM', data, \
                    student_n, item_n, knowledge_n, loss_func, \
                    knowledge_embed_size=EMBEDDING_SIZE, epoch_size=NUM_EPOCHS, \
    ...
    # logging.info("error on test data: {}".format(meta_learner.evaluate(test_data)))

    accuracy, precision, recall, f1, roc_auc = meta_learner.get_test_score(test_data)

    logging.info("ACDM | Rmse: {:4.6f} | Accuracy: {:4.6f} | Precision: {:4.6f} | Recall: {:4.6f} | F1: {:4.6f} | AUC: {:4.6f}"\
...
for i in range(TRAIN_NUM):
    ...
    logging.info("AGCDM | Rmse: {:4.6f} | Accuracy: {:4.6f} | Precision: {:4.6f} | Recall: {:4.6f} | F1: {:4.6f} | AUC: {:4.6f}"\
          .format(loss, accuracy, precision, recall, f1, roc_auc))
...
results1 = torch.load('./results/scores/Experiment1_NeuralCDM_' + 'FrcSub' + '.pt')
results2 = torch.load('./results/scores/Experiment1_ACDM_' + 'FrcSub' + '.pt')
results3 = torch.load('./results/scores/Experiment1_AGCDM_' + 'FrcSub' + '.pt')


logging.info('Rmse   | NeuralCDM: {:4.6f} | ACDM: {:4.6f} | AGCDM: {:4.6f}'.format(np.mean(results1['loss']), np.mean(results2['loss']), np.mean(results3['loss'])))
logging.info('Acc    | NeuralCDM: {:4.6f} | ACDM: {:4.6f} | AGCDM: {:4.6f}'.format(np.mean(results1['acc']), np.mean(results2['acc']), np.mean(results3['acc'])))
logging.info('F1     | NeuralCDM: {:4.6f} | ACDM: {:4.6f} | AGCDM: {:4.6f}'.format(np.mean(results1['f1']), np.mean(results2['f1']), np.mean(results3['f1'])))
logging.info('Recall | NeuralCDM: {:4.6f} | ACDM: {:4.6f} | AGCDM: {:4.6f}'.format(np.mean(results1['recall']), np.mean(results2['recall']), np.mean(results3['recall'])))
logging.info('AUC    | NeuralCDM: {:4.6f} | ACDM: {:4.6f} | AGCDM: {:4.6f}'.format(np.mean(results1['auc']), np.mean(results2['auc']), np.mean(results3['auc'])))
...
# NeuralCDM Train without meta
loss_func = nn.CrossEntropyLoss()
meta_learner = MetaLearner('NeuralCDM', data, \
                 student_n, item_n, knowledge_n, loss_func, \
                 knowledge_embed_size=EMBEDDING_SIZE, epoch_size=NUM_EPOCHS, \
                 batch_size=BATCH_SIZE, learning_rate = LEARNING_RATE)
meta_learner.reset_model()
meta_learner.train()
# meta_learner.show_train_val()
meta_learner.evaluate(test_data)
logging.info("RMSE on test data: {}".format(meta_learner.evaluate(test_data)))

accuracy, precision, recall, f1, roc_auc = meta_learner.get_test_score(test_data)
logging.info("NeuralCDM | Accuracy: {:4.6f} | Precision: {:4.6f} | Recall: {:4.6f} | F1: {:4.6f} | AUC: {:4.6f}"\
      .format(accuracy, precision, recall, f1, roc_auc))

# ACDM Train without meta
loss_func = nn.CrossEntropyLoss()
meta_learner = MetaLearner('ACDM', data, \
                 student_n, item_n, knowledge_n, loss_func, \
                 knowledge_embed_size=EMBEDDING_SIZE, epoch_size=NUM_EPOCHS, \
...
accuracy, precision, recall, f1, roc_auc = meta_learner.get_test_score(test_data)
logging.info("ACDM | Accuracy: {:4.6f} | Precision: {:4.6f} | Recall: {:4.6f} | F1: {:4.6f} | AUC: {:4.6f}"\
      .format(accuracy, precision, recall, f1, roc_auc))
...
accuracy, precision, recall, f1, roc_auc = meta_learner.get_test_score(test_data)
logging.info("AGCDM | Accuracy: {:4.6f} | Precision: {:4.6f} | Recall: {:4.6f} | F1: {:4.6f} | AUC: {:4.6f}"\
      .format(accuracy, precision, recall, f1, roc_auc))
...
train_losses, val_losses = meta_learner.train_losses.copy(), meta_learner.val_losses.copy()
